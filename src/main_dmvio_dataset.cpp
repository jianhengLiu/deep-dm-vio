...
void parseArgument(char *arg, dmvio::SettingsUtil &settingsUtil) {
  ...
  // --------------------------------------------------
  // These are mostly the original DSO commandline arguments which also work for
  // DM-VIO. The DM-VIO specific settings can also be set with commandline
  // arguments (and also with the yaml settings file) and have been registered
  // in the main function. See IMUSettings and its members for details.
  // --------------------------------------------------
...
void run(ImageFolderReader *reader, IOWrap::PangolinDSOViewer *viewer) {
  ...
  std::vector<double>
      timesToPlayAt; // 储存每个图片什么时候要被播放(根据playbackSpeed调节播放速度)
  for (int i = lstart;
  ...
  for (int ii = 0; ii < (int)idsToPlay.size(); ii++) {
    ...
    std::unique_ptr<dmvio::IMUData>
        imuData; // unique_ptr: 指向一个给定的对象，不支持拷贝和赋值
    if (setting_useIMU) {
    ...
    if (!skipFrame) {
      if (imuDataSkipped &&
          imuData) // 添加未处理的imu数据（即被skipFrame时的IMU信息）
      {
      ...
      fullSystem->addActiveFrame(img, i, imuData.get(),
                                 (gtDataThere && found) ? &data : nullptr);
  ...
  fullSystem->printResult(imuSettings.resultsPrefix + "result.txt", false,
                          false, true);
  fullSystem->printResult(imuSettings.resultsPrefix + "resultKFs.txt", true,
                          false, false);
...
int main(int argc, char **argv) {
  ...
  settingsUtil->registerArg("setting_minOptIterations",
                            setting_minOptIterations);
  settingsUtil->registerArg("setting_maxOptIterations",
  ...
  settingsUtil->registerArg("setting_weightZeroPriorDSOInitY",
                            setting_weightZeroPriorDSOInitY);
  settingsUtil->registerArg("setting_weightZeroPriorDSOInitX",
                            setting_weightZeroPriorDSOInitX);
  settingsUtil->registerArg("setting_forceNoKFTranslationThresh",
                            setting_forceNoKFTranslationThresh);
  settingsUtil->registerArg("setting_minFramesBetweenKeyframes",
                            setting_minFramesBetweenKeyframes);
  ...
  ImageFolderReader *reader =
      new ImageFolderReader(source, calib, gammaCalib, vignette, use16Bit);
  reader->loadIMUData(imuFile);
  reader->setGlobalCalibration();

  if (!disableAllDisplay) {
    IOWrap::PangolinDSOViewer *viewer =
        new IOWrap::PangolinDSOViewer(wG[0], hG[0], false, settingsUtil);
