...
bool CoarseInitializer::trackFrame(
  ...
  for (int lvl = pyrLevelsUsed - 1; lvl >= 0; lvl--) {
    ...
    while (true) {
      ...
      refToNew_new =
          SE3::exp(inc.head<6>().cast<double>()) * refToNew_current; // 更新位姿
...
Vec3f CoarseInitializer::calcResAndGS(int lvl, Mat88f &H_out, Vec8f &b_out,
  ...
  Eigen::Vector2f r2new_aff =
      Eigen::Vector2f(exp(refToNew_aff.a), refToNew_aff.b); // 光度参数
  ...
  for (int i = 0; i < npts; i++) {
    ...
    for (int idx = 0; idx < patternNum; idx++) {
      ...
      // 插值得到新图像中的 patch 像素值，(输入3维，输出3维[像素值, x方向梯度,
      // y方向梯度])
