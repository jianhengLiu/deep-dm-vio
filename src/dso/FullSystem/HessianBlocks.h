...
struct FrameHessian {
  ...
  Eigen::Vector3f
      *dI; //!< 图像导数// trace, fine tracking. Used for direction select (not
           // for gradient histograms etc.)
  Eigen::Vector3f *dIp[PYR_LEVELS]; //!< 各金字塔层的图像导数// coarse tracking
                                    //!< / coarse initializer. NAN in [0] only.
  float *absSquaredGrad[PYR_LEVELS]; //!< x,y 方向梯度的平方和// only used for
                                     //!< pixel select (histograms
                                     // etc.). no NAN.
