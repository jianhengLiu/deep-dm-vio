...
std::pair<Vec4, bool> FullSystem::trackNewCoarse(FrameHessian* fh, Sophus::SE3* referenceToFrameHint)
{
    ...
    // 寻找良好的初始化位姿,有IMU则用IMU推断结果放入lastF_2_fh_tries,若无则根据历史运动来假设各种运动可能放入lastF_2_fh_tries中
    // 利用IMU推断结果作为初始化位姿
    if (referenceToFrameHint) {
        ...
        if (allFrameHistory.size() == 2)
            ...
            { // lock on global pose consistency!
                ...
                lastF_2_slast = slast->camToWorld.inverse() * lastF->shell->camToWorld; // 参考帧到上一帧运动 // (T^w_t-1)^-1*T^w_t = T^t-1_t
                aff_last_2_l = slast->aff_g2l;
            }
...
void FullSystem::flagPointsForRemoval()
{
    ...
    // if(setting_margPointVisWindow>0)
    {
        ...
        for (auto& frameHessian : frameHessians)
            if (frameHessian->flaggedForMarginalization)
                fhsToMargPoints.push_back(frameHessian);
    }
    ...
    for (FrameHessian* host : frameHessians) // go through all active frames
    {
        for (unsigned int i = 0; i < host->pointHessians.size(); i++) {
            ...
            //* 丢掉相机后面, 没有残差的点
            if (ph->idepth_scaled < setting_minIdepth || ph->residuals.empty()) {
...
void FullSystem::initializeFromInitializer(FrameHessian* newFrame)
{
    ...
    firstFrame->idx = frameHessians.size(); // 赋值给它id (size=0开始)（滑窗id？）
    ...
    float rescaleFactor = 1 / (sumID / numID); // 求出尺度因子 //TODO: 这里的尺度因子后续改为双目进行选择?
