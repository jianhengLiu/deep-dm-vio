...
void FullSystem::printResult(std::string file, bool onlyLogKFPoses,
  ...
  for (FrameShell *s : allFrameHistory) {
    ...
    myfile << s->timestamp << " " << camToFirst.translation().transpose() << " "
...
void FullSystem::addActiveFrame(ImageAndExposure *image, int id,
  ...
  shell->incoming_id =
      id; // 记录进来图片在对应序列的id，仅用于信息输出；TODO:可以删除
  fh->shell = shell;
        ...
        0) // first frame set. fh is kept by coarseInitializer.
    {
      ...
      if (initDone) // if SNAPPED
      {
        ...
        if (timeBetweenFrames >
          ...
          // Clear frame history
          allFrameHistory.pop_back(); // Pop the current shell as we do not want
                                      // to delete it.
    ...
    if (setting_keyframesPerSecond > 0) {
      ...
      needToMakeKF =
          ...
          setting_kfGlobalWeight * setting_maxShiftWeightT *
                      sqrtf((double)tres[1]) / (wG[0] + hG[0]) // 平移像素位移
                  + setting_kfGlobalWeight * setting_maxShiftWeightR *
                        sqrtf((double)tres[2]) /
                        (wG[0] + hG[0]) // TODO 旋转像素位移, 设置为0???
              ...
              1 //光度变化大，TODO:光度
          || 2 * coarseTracker->firstCoarseRMSE <
                 tres[0] // 误差能量变化太大(最初的两倍)
          || (setting_maxTimeBetweenKeyframes > 0 &&
              timeSinceLastKeyframe >
                  setting_maxTimeBetweenKeyframes) // 关键帧间间隔时间大
          || forceKF;
...
void FullSystem::deliverTrackedFrame(FrameHessian *fh, bool needKF) {
  ...
  if (linearizeOperation) {
    ...
    while (coarseTracker_forNewKF->refFrameID == -1 &&
      ...
      mappedFrameSignal.wait(
          lock); // 当没有跟踪的图像, 就一直阻塞trackMapSyncMutex, 直到notify
    }
