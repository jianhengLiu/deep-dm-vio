...
/**
 * @brief //TODO 使用旋转和位移对像素移动的作用比来判断运动状态
 * 
 * @param fh 
 * @param referenceToFrameHint 
 * @return std::pair<Vec4, bool> 
 */
...
void FullSystem::addActiveFrame(ImageAndExposure* image, int id, dmvio::IMUData* imuData, dmvio::GTData* gtData)
{
    ...
    } else // do front-end operation.
    {
        ...
        if (setting_keyframesPerSecond > 0) {
            ...
            // BRIGHTNESS CHECK
            needToMakeKF = allFrameHistory.size() == 1 || setting_kfGlobalWeight * setting_maxShiftWeightT * sqrtf((double)tres[1]) / (wG[0] + hG[0]) + setting_kfGlobalWeight * setting_maxShiftWeightR * sqrtf((double)tres[2]) / (wG[0] + hG[0]) + setting_kfGlobalWeight * setting_maxShiftWeightRT * sqrtf((double)tres[3]) / (wG[0] + hG[0]) + setting_kfGlobalWeight * setting_maxAffineWeight * fabs(logf((float)refToFh[0])) > 1 || 2 * coarseTracker->firstCoarseRMSE < tres[0] || (setting_maxTimeBetweenKeyframes > 0 && timeSinceLastKeyframe > setting_maxTimeBetweenKeyframes) || forceKF;
